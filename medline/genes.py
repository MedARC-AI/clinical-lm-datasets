...
if __name__ == '__main__':
    # This is "A" genes
    links = ['https://medlineplus.gov/genetics/gene/']
    for l in list(string.ascii_lowercase)[1:]:
        links.append(f'https://medlineplus.gov/genetics/gene-{l}')
    ...
    for link in links:
        ...
        for a in soup.find_all('a'):
            ct += 1
            gene_links.add(a.get('href'))
            link2gene[a.get('href')] = a.text.strip()
        
        print(f'{link} -> {ct}')

    dataset = []
    seen = set()
    for link in tqdm(sorted(list(gene_links))):
        gene = link2gene[link]
        response = requests.get(link)
        html_content = response.text
        ...
        for idx, chapter in enumerate(chapters):
            ...
            if 'health conditions related to' in header.lower():
                continue
    ...
    out_dir = os.path.join(OUT_DIR, 'genes_hf')
    print(f'Saving {len(dataset)} gene articles to {out_dir}')
