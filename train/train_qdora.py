...
def run_validation(args, model, local_rank, val_dataloader, autocast):
    val_losses = torch.zeros(2).to(local_rank)
...
# And to get the dataloader
def get_hf_dataloader(args:Dict, split_data, tokenizer, split: str = None):
    """Creates a dataset and appropriate dataloader with distributed sampler."""
...
def get_hf_mcqa_dataloader(args:Dict, split_data, tokenizer, split: str = None):
    """Creates a dataset and appropriate dataloader with distributed sampler."""
    bs = args["batch_size"] if split == 'train' else args["eval_batch_size"]
...
def fsdp_main(local_rank:int, world_size:int, args:Dict):
    ...
    if args["train_mode"] == "pretrain":
        ...
        val_dataloader, sources = get_hf_mcqa_dataloader(args, val, tokenizer, split="validation")  # global_rank, world_size, 
        print(sources)
        ...
        dataloader, _ = get_hf_dataloader(args, train, tokenizer, split="train")
        val_dataloader, sources = get_hf_dataloader(args, val, tokenizer, split="validation")
        print(sources)
        args["sources"] = sources
        ...
        train = filter_by_source(dataset['train'], args['train_source_filters'])
        val = filter_by_source(dataset['validation'], args['validation_source_filters'])
        dataloader, _ = get_hf_mcqa_dataloader(args, train, tokenizer, split="train")
        val_dataloader, sources = get_hf_mcqa_dataloader(args, val, tokenizer, split="validation")
        print(sources)
        args["sources"] = sources
    else:
                ...
                if args["save_model"] and steps % args["save_steps"] == 0:
                    if val_dataloader is not None:
                        ...
                        if args["train_mode"] in {'finetune_mcqa', 'pretrain'}:
                            ...
                            if rank == 0:
                                if args["log_to"] == 'wandb':
                                    logger.log({f"validation/loss": val_loss}, rank)

                                print(f"Resuming training from step #{steps}...")
                        model.train()
                    ckpt_fn = save_checkpoint(args, model, new_layer_names, rank, steps)
    ...
    if args["save_model"]:
        ckpt_fn = save_checkpoint(args, model, new_layer_names, rank, 'final')
...
def main(
    ...
    dataset_samples: int = 99999999999999999999, # Number of samples in an epoch if using "alpaca_sample" or "dummy" dataset
