...
def pack(args):
    ...
    shape = (len(reweighted_dataset), 8192)  # 4 million rows, 8192 columns
    ...
    for i, item in tqdm(enumerate(reweighted_dataset), total=len(reweighted_dataset)):
        # Assuming your dataset is structured with the key 'input_ids' for accessing the data
        # Adjust the key as necessary
        fp[i] = item['input_ids']
...
if __name__ == '__main__':
    ...
    if args.all_configs:
        ...
        #     # else:
        #     args.out_dir = os.path.join(args.tokenized_dir, 'tokenized', '{args.reweighting_config}')
